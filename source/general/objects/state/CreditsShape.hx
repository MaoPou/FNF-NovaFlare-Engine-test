...
class ModsButtonRect extends FlxSpriteGroup // play/back button
{
	...
	public function new(X:Float, Y:Float, width:Float = 0, height:Float = 0, roundWidth:Float = 0, roundHeight:Float = 0, texts:String = '',
			?specPath:Bool = false, textOffset:Float = 0, color:FlxColor = FlxColor.WHITE, onClick:Void->Void = null)
	{
		super(X, Y);
		...
		if (bmp != null)
		{
			...
			box.loadGraphic(Paths.image('unknownMod'), true, 150, 150);
		...
		if (text.width > width - box.width - 20)
			text.scale.x = (width - box.width - 20) / text.width;
...
class CreditsNote extends FlxSprite
{
	...
	public function changeIcon(char:String, ?allowGPU:Bool = true)
	{
		...
		if (char == "github")
			animation.curAnim.curFrame = 0;
		else if (char == "youtube")
			animation.curAnim.curFrame = 1;
		else if (char == "x.com" || char == "twitter")
			animation.curAnim.curFrame = 2;
		else if (char == "discord")
			animation.curAnim.curFrame = 3;
		else if (char == "bilibili" || char == "b23.tv")
			animation.curAnim.curFrame = 4;
		else if (char == "douyin")
			animation.curAnim.curFrame = 5;
		else if (char == "kuaishou")
			animation.curAnim.curFrame = 6;
		else
			animation.curAnim.curFrame = 7;
	}
