...
class ColorSwapShader extends FlxShader
{
	@:glFragmentSource('
		...
		vec4 flixel_texture2D(sampler2D bitmap, vec2 coord)
		{
			...
			vec3 rgb = color.rgb;
			float alpha = color.a;
			bool needsDivision = alpha < 1.0;
			
			rgb = mix(rgb, rgb / alpha, float(needsDivision));
			
			vec3 colorMult = openfl_ColorMultiplierv.xyz;
			float alphaMult = openfl_ColorMultiplierv.w;
			
			rgb = clamp(openfl_ColorOffsetv.xyz + rgb * colorMult, 0.0, 1.0);
			alpha = clamp(openfl_ColorOffsetv.w + alpha * alphaMult, 0.0, 1.0);
			
			float finalAlpha = alpha * openfl_Alphav;
