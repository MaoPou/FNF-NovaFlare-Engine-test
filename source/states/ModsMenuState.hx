...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		modRestartText = new FlxText(bgDescription.x + 15, bgDescription.y + bgDescription.height - myHeight - 25, bgDescription.width - 30,
			"* Moving or Toggling On/Off this Mod will restart the game.", 16);
		...
		var button = new MenuButton(buttonsX + 90, buttonsY, 80, 80, Paths.image('modsMenuButtons'), function() moveModToPosition(modsList.all.length - 1),
			54, 54); // Move to the down
		button.icon.animation.add('icon', [1]);
		button.icon.animation.play('icon', true);
		add(button);
		buttons.push(button);

		var button = new MenuButton(buttonsX + 180, buttonsY, 80, 80, Paths.image('modsMenuButtons'), function() moveModToPosition(curSelectedMod - 1), 54,
			54); // Move up
		...
		var button = new MenuButton(buttonsX + 270, buttonsY, 80, 80, Paths.image('modsMenuButtons'), function() moveModToPosition(curSelectedMod + 1), 54,
			54); // Move down
